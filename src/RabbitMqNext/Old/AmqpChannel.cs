namespace RabbitMqNext
{
	...
	public class AmqpChannel : CommonCommandSender, IAmqpChannel
	{
...
//		private async Task DrainMethodsWithErrorAndClose(AmqpError error, ushort classId, ushort methodId)
//		{
...
//		}
		...
		public void Return(BasicProperties prop)
		{
		...
		private async Task InternalClose(bool sendClose)
		{
			...
			this.InitiateCleanClose(false, 0, 0 );
		...
		internal override async Task HandleFrame(int classMethodId)
		{
			switch (classMethodId)
			{
				case AmqpClassMethodChannelLevelConstants.ChannelClose:
					_connection._frameReader.Read_Channel_Close2(base.HandleCloseMethod);
					break;
